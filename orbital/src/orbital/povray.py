...
def create_pov( pin, fam_lst = [], show_surf = False, ani = False, ft_lst = [] ):
    ...
    INPUT:
        ...
        - "fam_lst"   -- A list of strings denoting family id's.
        
        - "show_surf" -- Boolean.     
        
        - "ani"       -- Boolean. If true then output animated gif.        

        - "ft_lst"    -- A list of pairs: [(<fam>,<t>)]
                         where <fam> is a string denoting a family id
                         and <t> is a float in [0,1] representing
                         transparancy.
...
def create_pov_preamble_declares( pin ):
    '''
    INPUT:
        - "pin" -- PovInput object where the following 
                   attributes are set:                                
                        "pin.cam_dct"
                        "pin.light_radius"
                        "pin.axes_dct"
...
def create_pov_preamble_camera():
    '''
    OUTPUT
        - A string of global settings, camera and light source 
          in povray format.
    '''
...
def create_pov_preamble( pin ):
    '''
    INPUT:
        - "pin" -- PovInput object is passed to 
                   "create_pov_preamble_declares()".
    OUTPUT
...
def create_pov_axes():
    '''
    OUTPUT:
        - Returns a string of axes in povray format.
    '''
...
def create_pov_curves( pin, fam, num = -1 ):
    '''
    INPUT:
        - "pin" -- PovInput object, which is passed to "povray_aux.get_curve_lst()".
        - "fam" -- A string key denoting a family id (eg. 'A'). 
        - "num" -- An integer >= -1. 
    OUTPUT
...
def pov_raytrace( pin, pov_str ):
    ...
    INPUT:
        - "pin"     -- PovInput object, where the following
                       attributes are used:
                           ...
                           * pin.height
                           * pin.quality
        - "pov_str" -- A string of a povray input file.
    OUTPUT
    ...
    # write povray string
    #
    op( 'Writing povray string to:', file_name )
...
def pov_ani( pin, fam, show_surf = False, ft_lst = [] ):
    '''
    INPUT:
        - "pin"       -- PovInput object using:
                            * "pin.path"
                            * "pin.fname"
                            * "pin.ani_delay"
                                                
        - "fam"       -- A string key for the family which 
                         should be animated. 
                 
        - "show_surf" -- If True then the surface on which the
                         family lives is rendered.
    ...
    OUTPUT:
        - A animated gif at "pin.path".
          In each frame, all curves in family <fam> are 
          rendered with transparancy <t>, 
          for all (<fam>,<t>) in "ft_lst", 
          such that <fam> is not the family that is 
          animated.  
    '''
